...
public class LaptopService extends LaptopServiceGrpc.LaptopServiceImplBase {
  ...
  public void syncDataToElasticSearch(Laptop laptop) {
    String jsonLaptop = JsonHelper.mapLaptopProtoToJson(laptop);
    JavaElasticSearch.pushData("bao", jsonLaptop);
  }
  ...
  public void advancedSearch(SearchLaptopRequest request, StreamObserver<SearchLaptopResponse> responseObserver) {
    ...
    SearchSourceBuilder searchSourceBuilder = new SearchSourceBuilder().from(0).size(10);
    searchSourceBuilder.query(SearchBuildHelper.buildQueryBuilder(filter));
    SearchResult<com.kegmil.example.pcbook.data.Laptop> result = JavaElasticSearch.search("bao", searchSourceBuilder, com.kegmil.example.pcbook.data.Laptop.class);
